...
export function uptime(seconds: number): string {
  if (!seconds) return "—"
  // Guard: if value looks like an epoch timestamp (> ~10 years in seconds),
  // convert it to uptime by subtracting current time
  let s = seconds
  if (s > 315360000) {
    // Likely epoch timestamp, compute delta
    const now = Date.now() / 1000
    s = Math.max(0, now - s)
    if (s > 315360000) s = 0 // Still unreasonable, bail
  }
  if (s <= 0) return "—"
...
export function daysUntil(date?: string | number | null): number | null {
  if (!date) return null
  if (typeof date === "number") {
    const ms = date > 1e11 ? date : date * 1000
  ...
  if (Number.isNaN(target)) {
    target = new Date(clean.replace(" ", "T")).getTime()
  }
...
/** Convert 2-letter country code into native Unicode emoji flag (e.g. US -> 🇺🇸, JP -> 🇯🇵) */
